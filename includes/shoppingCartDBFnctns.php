...
function getCustomerID($username, $password)
{
	...
	$myQuery .= " FROM factandfictionB.customer ";
...
{
	# set up query for customer id

	$myQuery = "SELECT o_id, c_id";
	$myQuery .= " FROM factandfictionB.order ";
	$myQuery .= " WHERE c_id = ? ";
...
{
	# set up query for customer order update
	
	$total = $subTotal +  $salesTax + $shipping;

	$myQuery = "Update factandfictionB.order ";
...
function addCustOrder($c_id)
{
	...
	$myNewOrder = "Insert into factandfictionB.order (c_id, dateAdded) ";
	$myNewOrder .= " values (?, ?)";																	
...
{	
	...
	$myOrderItem = "Insert into factandfictionB.orderitem (o_id, title_id, quantity, dateAdded ) ";
	$myOrderItem .= " values (?, ?, ?, ?);";																	
...
function updateOrderItemQantity($oi_id, $quantity)
{
   ...
   # set up update of order item quantity
	...
	$myUpdOrderItem = "Update factandfictionB.orderitem ";
...
function getOrderItemQuantity($o_id, $title_id)
{
	...
	$myQuery .=  " FROM factandfictionB.orderitem ";
	$myQuery .=  " WHERE o_id = ? ";
...
function get_title_info(&$title_info)
{
	...
	$myQuery .=  "FROM factandfictionB.titles ";
...
{		
	# set up update
	$myQuery = "Update factandfictionB.titles ";
...
# set up query for address
	...
	$myQuery .=  "FROM factandfictionB.custAddress ";
	$myQuery .=  "where c_id = ? ";
...
# set up address insert
	$myQuery = "INSERT INTO factandfictionB.custAddress (c_id, ca_addresstype, ca_streetaddress, ca_city, ";
	$myQuery .=  " ca_state_id, ca_zip, ca_email, ca_phone, ca_cell )";
	$myQuery .=  "VALUES ( ?, ?, ?, ?, ?, ?, ?, ?, ?);";
